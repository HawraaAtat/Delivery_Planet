...
<div class="table-responsive pb-5">
    <table id="tbOrderHistory" class="table border ps-table w-100 mb-3">
        <thead>
            <tr>
                ...
                <th class="font-weight-bold py-2 border-0 quantity" style="color: #fea116;">Plate name</th>
                <th class="font-weight-bold py-2 border-0 " style="color: #fea116;">Quantity</th>
        ...
        <tbody>
            @foreach ($orderItem as $rec)
                
           
            <tr>
                <th class="font-weight-bold py-2 border-0">{{$rec->order_id}}</th>
                <th class="font-weight-bold py-2 border-0 quantity">{{$rec->menu_name}}</th>
                <th class="font-weight-bold py-2 border-0 ">{{$rec->quantity}}</th>
                <th class="font-weight-bold py-2 border-0 ">{{$rec->price}}</th>
                <th class="font-weight-bold py-2 border-0 ">{{$rec->price * $rec->quantity}}</th>
    ...
    <form action="{{ route('order.edit', ['order'=>$orderr->id] ) }}" class="login-form" method="post">
        @method('GET')
        @csrf
        <button type="submit" class="btn btn-primary" style="font-size: 12px"><i class="bi bi-backspace-fill"></i> Back to order</button>
    </form>
    </div>
