...
<div class="container">
    <div class="text-center wow fadeInUp" data-wow-delay="0.1s">
        <h5 class="section-title ff-secondary text-center text-primary fw-normal">edit Plate</h5>
        <br>
        <br>
    </div>
    <div class="input-group col-lg-10 col-md-12" style=" justify-content: center; align-items: center;">
            ...
            <div class="small-container " style="justify-content: center; align-items: center;">
              ...
              <form action="{{ Route(  "menu_add.update" , ['menu_add'=>$menu->id] ) }}" method="post" enctype="multipart/form-data">
                @csrf
                @method("PUT")
                <input type="text" name="menu_name" placeholder="Name" value="{{$menu->menu_name}}"/>
                @error('menu_name')
                <label style="color: red">{{$message}}</label>
                @enderror
                <input type="text" name="price" placeholder="Price" value="{{$menu->price}}"/>
                @error('price')
                <label style="color: red">{{$message}}</label>
                @enderror
                <input type="text" name="calorie_count" placeholder="Calorie Count" value="{{$menu->calorie_count}}"/>
                @error('calorie_count')
                <label style="color: red">{{$message}}</label>
                @enderror
                ...
                <div class="input-group-append input">
                    <select class="form-control input-group-prepend btn btn-secondary py-2 px-4" id="search-filter" name="cuisine">
                        <option value="{{$menu->cuisine}}">{{$menu->cuisine}}</option>
                ...
                @enderror
                <div class="input-group col-lg-10 col-md-12" style=" justify-content: center; align-items: center;">
                <button name="submit" type="submit" id="submit" class="btn btn-primary btn-block btn-large">Edit Plate</button>
